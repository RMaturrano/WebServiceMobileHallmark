...
try{
	...
	if (id !== undefined)
	{
        ...
        var query = 'select IFNULL("ClaveMovil",\'\') AS "ClaveMovil" , ' +
                        ...
                        '    IFNULL(TO_VARCHAR("Comentario"),\'\') AS "Comentario" , ' +
                    ...
                    ' from '+Constants.BD_MOBILE+'."ORDR" where "Migrado" = \'N\' AND "EMPRESA" = ' + id;
    	...
    	if (rs.length > 0)
    	{
    		...
    		for(i = 0; i < rs.length ; i++)
    		{
        		...
        		salesOrder += '"Comentario": "'+ functions.CleanChars(rs[i].Comentario) +'",';
        		...
        		salesOrder += '"Descuento": '+rs[i].Descuento+',';
            	...
            	try{
            	    //var mSO = JSON.parse(salesOrder);
            	    //salesOrder += '"Comentario": "'+ functions.CleanChars(rs[i].Comentario) +'",';
            	    //mSO.Comentario = rs[i].Comentario;
            	    //mResult.push(mSO);
            	    
            	    mResult.push(JSON.parse(salesOrder));
            	}catch(e){
            	    //throw new functions.buildException(salesOrder);
            	}
    		}
