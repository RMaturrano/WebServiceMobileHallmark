	...
	if (id !== undefined)
	{
	    ...
	    if(migrado !== undefined && migrado !== ''){
         ...
         query = 'select IFNULL("ClaveMovil",\'\') AS "ClaveMovil" , ' +
                        ...
                        '    IFNULL("NombreComercial",\'\') AS "NombreComercial" , ' +
                        '    IFNULL("ApellidoPaterno",\'\') AS "ApellidoPaterno" , ' +
                    ...
                    ' from '+Constants.BD_MOBILE+'."OCRD" where "EMPRESA" = ' + id.toString() + qwhere.toString();
